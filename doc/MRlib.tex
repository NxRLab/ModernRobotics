...
{d = DistanceToSO3(A)}
{\item \verb~A~: A $3 \times 3$ matrix $A$.}
...
{val = TestIfSO3(A)}
{\item \verb~A~: A $3 \times 3$ matrix $A$.}
{\item \verb~val~:  $1$ if $A$ is a rotation matrix (an element of $SO(3)$) and $0$ otherwise.  This function calls \verb~DistanceToSO3(A)~ and tests if the returned distance is smaller than a small value (which you should feel free to change to suit your purposes).}
{}
\end{function}
...
{val = TestIfSE3(A)}
{\item \verb~A~: A $4 \times 4$ matrix $A$.}
{\item \verb~val~:  $1$ if $A$ is a transformation matrix (an element of $SE(3)$) and $0$ otherwise.  This function calls \verb~DistanceToSE3(A)~ and tests if the returned distance is smaller than a small value (which you should feel free to change to suit your purposes).}
{}
\end{function}
